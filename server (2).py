...
async def register(payload: RegisterIn, response: Response):
    ...
    if not settings.get("registration_enabled", False):
        raise HTTPException(status_code=403, detail="El registro público está desactivado. Contacta al administrador para obtener acceso.")
    email = payload.email.lower().strip()
    if await db.users.find_one({"email": email}):
        raise HTTPException(status_code=400, detail="El email ya está registrado")
    require_approval = settings.get("require_approval", True)
    status_val = "pending" if require_approval else "active"
    doc = {
        "email": email,
    ...
    if require_approval:
        raise HTTPException(status_code=202, detail="Cuenta creada. Tu acceso está pendiente de aprobación por el administrador.")
    access = create_access_token(user_id, email)
    set_auth_cookies(response, access, create_refresh_token(user_id))
    return AuthResponse(
        user=UserPublic(id=user_id, email=email, name=doc["name"], role="user", plan="free"),
        access_token=access,
    )
...
async def login(payload: LoginIn, request: Request, response: Response):
    ...
    if user.get("status") == "pending":
        raise HTTPException(status_code=403, detail="Tu cuenta está pendiente de aprobación. Contacta al administrador.")
    ...
    return AuthResponse(
        user=UserPublic(id=uid, email=email, name=user.get("name", ""), role=user.get("role", "user"), plan=user.get("plan", "free")),
        access_token=access,
    )
...
async def me(user: dict = Depends(get_current_user)):
    return UserPublic(id=user["id"], email=user["email"], name=user.get("name", ""), role=user.get("role", "user"), plan=user.get("plan", "free"))
...
# ---------- Courses ----------
LEVEL_ORDER = {"principiante": 0, "intermedio": 1, "avanzado": 2}
...
async def list_courses(user: dict = Depends(get_current_user)):
    ...
    async for c in cursor:
        ...
        course_level = c.get("level", "principiante")
        plan_order = {"free": 0, "principiante": 0, "intermedio": 1, "avanzado": 2}
        level_order = {"principiante": 0, "intermedio": 1, "avanzado": 2}
        user_level = plan_order.get(user_plan, 0)
        req_level = level_order.get(course_level, 0)
        c["locked"] = req_level > user_level and user.get("role") != "admin"
        courses.append(c)
    level_order_map = {"principiante": 0, "intermedio": 1, "avanzado": 2}
    courses.sort(key=lambda c: level_order_map.get(c.get("level", "principiante"), 0))
    return courses
...
# ---------- Stats ----------
@api.get("/admin/stats")
async def get_stats(current: dict = Depends(require_admin)):
    from datetime import datetime, timezone, timedelta
    ...
    # Users by plan
    plan_counts = {"free": 0, "intermedio": 0, "avanzado": 0}
...
async def update_user_status(user_id: str, payload: StatusUpdate, current: dict = Depends(require_admin)):
    if payload.status not in ("active", "pending", "blocked"):
        raise HTTPException(status_code=400, detail="Estado inválido")
    try:
        oid = ObjectId(user_id)
    except Exception:
        raise HTTPException(status_code=400, detail="ID inválido")
    await db.users.update_one({"_id": oid}, {"$set": {"status": payload.status}})
    return {"ok": True, "status": payload.status}

# ---------- User Plan Management ----------
class PlanUpdate(BaseModel):
    plan: str  # "free" | "intermedio" | "avanzado"

@api.patch("/admin/users/{user_id}/plan")
async def update_user_plan(user_id: str, payload: PlanUpdate, current: dict = Depends(require_admin)):
    if payload.plan not in ("free", "intermedio", "avanzado"):
        raise HTTPException(status_code=400, detail="Plan inválido")
    try:
        oid = ObjectId(user_id)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"ID inválido: {e}")
    logger.info(f"Updating plan for user {user_id} to {payload.plan}")
    user_doc = await db.users.find_one({"_id": oid})
    if not user_doc:
        raise HTTPException(status_code=404, detail=f"Usuario no encontrado con id: {user_id}")
    await db.users.update_one({"_id": oid}, {"$set": {"plan": payload.plan}})
...
async def upload_file(file: UploadFile = File(...), _: dict = Depends(require_admin)):
    ...
    if len(data) > MAX_SIZE:
        raise HTTPException(400, "Imagen muy grande. Máximo 5 MB.")
